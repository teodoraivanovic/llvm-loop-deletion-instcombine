...
PreservedAnalyses CustomLoopDeletionPass::run(Loop &L, LoopAnalysisManager &LAM,
    ...
    for (BasicBlock *BB : L.blocks()) {
        for (Instruction &I : *BB) {
            ...
            if (auto *SI = dyn_cast<StoreInst>(&I)) {
                ...
                // "underlying object" - separate allocated memory region that a
                // pointer is based on
                Value *Base = getUnderlyingObject(Ptr);

                // if base is a local alloca => safe
                if (isa<AllocaInst>(Base)) {
                    errs() << "*** Has a local store, but values are not used "
                              "outside of the loop\n";
                    continue;
                }
...
bool CustomLoopDeletionPass::hasLiveExitValues(Loop &L) {
    for (BasicBlock *BB : L.blocks()) {
        for (Instruction &I : *BB) {
            ...
            for (Use &U : I.uses()) {
                if (Instruction *User = dyn_cast<Instruction>(
                        U.getUser())) { // if it's not a real instruction but a
...
llvmGetPassPluginInfo() {
    return {
        LLVM_PLUGIN_API_VERSION, "CustomLoopDeletion", LLVM_VERSION_STRING,
        [](PassBuilder &PB) {
